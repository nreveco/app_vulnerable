@echo off
REM Blue Team Remediation Script for Vulnerable Spring Boot App (Windows)

echo 🔵 BLUE TEAM: Starting security remediation process...
echo ==================================================

REM Step 1: Backup current vulnerable configuration
echo 📋 Step 1: Backing up vulnerable configuration
copy vulnerable-app\pom.xml vulnerable-app\pom-vulnerable-backup.xml
echo ✅ Backup created: pom-vulnerable-backup.xml

REM Step 2: Apply security fixes
echo 🔧 Step 2: Applying security fixes
copy vulnerable-app\pom-secure.xml vulnerable-app\pom.xml
echo ✅ Updated pom.xml with secure dependencies

REM Step 3: Run security scans
echo 🔍 Step 3: Running OWASP Dependency Check
cd vulnerable-app
call mvn dependency-check:check -DfailBuildOnCVSS=0
echo ✅ Security scan completed

REM Step 4: Generate security report
echo 📊 Step 4: Generating security reports
call mvn compile
call mvn dependency-check:check -Dformat=ALL
echo ✅ Security reports generated in target\security-reports\

REM Step 5: Build secure version
echo 🏗️ Step 5: Building secure application
call mvn clean package -DskipTests
echo ✅ Secure application built

REM Step 6: Update Docker image
